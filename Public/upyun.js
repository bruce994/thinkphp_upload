...
function upyunWapPicUpload(domid,width,height,token,type){
	art.dialog.data('width', width);
	art.dialog.data('height', height);
	art.dialog.data('domid', domid);
	art.dialog.data('lastpic', $('#'+domid).val());
	art.dialog.open('/Home/index.php?m=Upyun&a=upload&from=Wap&token='+token+'&type='+type+'&width='+width,{lock:true,title:'上传图片',width:260,height:150,top:100,yesText:'关闭',background: '#000',opacity: 0.65});
}
